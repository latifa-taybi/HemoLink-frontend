...
export class PersonnelOverviewComponent implements OnInit {
  ...
  chargerKpis(): void {
    // Toujours charger le profil frais depuis le backend (évite les données locales périmées sans centreId)
    this.authService.fetchUserData().subscribe({
      ...
      error: (err) => {
        console.error('Erreur récupération user profile:', err);
        // Fallback just in case
        const userCached = this.authService.getUser();
        if (userCached) {
            // retry with cached
        } else {
            this.error = 'Impossible de vérifier votre profil sur le serveur. Veuillez vous reconnecter.';
            this.loading = false;
            this.cdr.markForCheck();
        }
      }
